...
/mob/living/carbon/human/contaminate()
	...
	if(!pl_head_protected())
		if(prob(0)) suit_contamination() //Plasma can sometimes get through such an open suit.
...
/mob/living/carbon/human/proc/pl_suit_protected()
	...
	if(wear_suit)
		if(zas_settings.Get(/datum/ZAS_Setting/PLASMAGUARD_ONLY))
			if(wear_suit.flags & PLASMAGUARD) return 0
		else
			if(is_slot_hidden(wear_suit.body_parts_covered,HIDEJUMPSUIT)) return 0
	return 0
